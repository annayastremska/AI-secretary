...
**2. Власний номер -- ОСТАННІЙ у титульному блоці, перед видавцем.** Не
перший. Доказ у даних: документ 202 називається «Методичні вказівки ... з
вимогами НД ТЗІ 2.5-004-99», тобто цитований стандарт входить у його ВЛАСНУ
назву на позиції 232, а власний номер `2.7-013-2016` стоїть на 1139 -- перед
видавцем. Правило «перший» присвоїло б йому чужий номер і поставило б поруч зі
справжнім 2.5-004-99 (документ 234).
...
## Чого тут принципово немає

Номера закону. Закон свого номера в тексті НЕ несе -- ні `550-XIV`, ні
`2011-XII` там немає, вони живуть лише в URL на zakon.rada. Тому для законів
ідентифікатор -- це посилання на ВВР (`ВВР 1999 № 22-23 ст.196`), яке в тексті
є і яке однозначне. Номер закону підв'язується окремо, зі звіреного вручну
списку `docs/tasks/2026-08-21_normative-corpus.md`, а не вгадується з тексту.
"""
import argparse
import os
import re
...
APPROVED_BY = re.compile(
    r"ЗАТВЕРДЖЕНО\s+Наказ\s+([^\n]{0,60}?)\s*(\d{1,2}\s+\w+\s+\d{4})\s*№\s*(\d+)", re.I)
...
def extract(text):
    ...
    if v:
        ...
        ident = f"ВВР {v.group(1)} № {v.group(2)} ст.{v.group(3)}"
        if not title:
            return dict(kind="law", identifier=ident, title=None, issue_date=None,
                        confidence=0.3,
                        reason="назви перед посиланням на ВВР немає -- титул втрачено при вивантаженні")
        return dict(kind="law", identifier=ident, title=title,
                    issue_date=None, confidence=0.85, reason=None)

    # --- Указ Президента ----------------------------------------------------
    if DECREE.search(head[:200]):
        tm = re.search(r"(Про\s+.{10,400})", head, re.S)
        return dict(kind="decree", identifier=None,
                    title=cut_title(tm.group(1)) if tm else None,
                    issue_date=None, confidence=0.4,
                    reason="указ без номера в тексті -- номер лише в URL")
    ...
    if a:
        ...
        return dict(kind="approved_doc",
                    identifier=f"затв. наказом № {a.group(3)} від {a.group(2)}",
                    title=cut_title(tm.group(1)) if tm else None,
                    issue_date=None, confidence=0.7, reason=None)

    # --- Лише назва ---------------------------------------------------------
    tm = re.search(r"^\s*((?:Про|Деякі|Щодо)\s+.{10,400})", head, re.S)
    if tm:
        return dict(kind="untitled_order", identifier=None,
                    title=cut_title(tm.group(1)), issue_date=None,
                    confidence=0.45,
                    reason="є назва, але немає номера й дати в тексті")
