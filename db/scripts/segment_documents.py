...
MARKERS = [
    ...
    #
    # «Додаток:» із двокрапкою -- це реквізит листа («Додаток: на 5 арк. у 2
    # прим.», «Додаток: 093c7b5d….zip»), а не заголовок. Він лишається МЕЖЕЮ
    # (реквізити справді відділені від тіла), але його текст не йде в мітку --
    # це робиться нижче, у місці, де мітка будується.
    #
    # Спершу я викинув такий рядок із маркерів узагалі. Адреси стали чисті
    # (490 одиниць із 12123 мали «Додаток:» у мітці -> 0), але разом із текстом
    # зникла й межа, і на наборі Дениса перестало знаходитись N15:
    # 12/15 -> 11/15, відтворено двічі в одному інстансі. Тому межа повернута.
    ...
    * `nest` -- те саме спостереження, але межа не відкидається, а стає
    ...
    for kind, pat in MARKERS:
        for m in pat.finditer(text):
            ...
            # «Додаток:» -- це реквізит листа («Додаток: на 5 арк. у 2 прим.»,
            # «Додаток: 093c7b5d….zip»), а не заголовок. МЕЖУ він задає
            # справжню (реквізити відділені від тіла), тому лишається
            # маркером, але його текст у мітку не йде: інакше адреса цитати
            # виходить «Додаток: …zip / 194».
            #
            # Спершу я прибрав сам маркер -- адреси стали чисті (490 -> 0),
            # але зникла межа, і на наборі Дениса N15 перестав знаходитись
            # (12/15 -> 11/15, відтворювано двічі). Тому межа лишається, а
            # чиститься лише мітка.
            if re.match(r"(?i)^Додаток\s*:", label):
                label = "Додаток (реквізити)"
