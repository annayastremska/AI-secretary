...
def _geometric_candidate(blocks, label_i, h_med):
    ...
    # "НАД" і "ПІД" визначаються ПОРЯДКОМ ОБОХ МЕЖ, а не допуском на дотик.
    #
    # Історія цього місця -- історія одного порогу, який двічі виявився замалим.
    # Спершу стояло строге `by2 <= ly1`, і рамка значення, що заходить на рамку
    # свого лейбла на ДВА пікселі (LEAVE-006.png: "старший сержант ВЛОХ..."
    # y 643-685 проти лейбла y 683-727), викидалась із розгляду ВЗАГАЛІ -- вона
    # не "праворуч", не "над", не "під", -- і найближчим зверху ставав номер
    # документа рядком вище. Тоді додався допуск `0.25 * h_med`, відкалібрований
    # на тих двох пікселях.
    #
    # ЗАМІРЯНО 23.08.2026 на фото демо-набору (DEMO-02.jpg): на знімку рамки
    # сусідніх рядків перетинаються не на два пікселі, а на третину рядка --
    # ПІБ 577-622 проти лейбла 608-656 (14 px) і посада 681-723 проти лейбла
    # 707-745 (16 px) при `0.25 * h_med` = 9 px. Обидва правильні кандидати
    # знову випадали з розгляду, і на місце посади ставав ЛЕЙБЛ попереднього
    # поля -- друкований текст бланка, через що поле оголошувалось "доведено
    # порожнім" (див. weak-spots 11.1).
    #
    # Тому порогу тут більше немає. "Над" = обидві межі кандидата вище обох
    # меж лейбла; "під" = обидві нижче. Це масштабно-незалежно (не залежить ні
    # від h_med, ні від того, наскільки OCR роздув рамку) і зберігає сенс
    # напрямку: справжній сусід по РЯДКУ має майже ті самі y-межі й уже
    # розібраний гілкою same_row вище, а блок, що ОХОПЛЮЄ лейбл (межа вище й
    # межа нижче), не є ні "над", ні "під" -- і саме таким і мусить лишитись.
    ...
    for i, block in enumerate(blocks):
        ...
        elif x_overlap and by1 < ly1 and by2 < ly2:
...
def proves_empty(reason, candidate) -> bool:
    """Чи справді ця причина ДОВОДИТЬ порожнечу слота, а не видає промах.

    Причина існування (заміряно 23.08.2026 на DEMO-02.jpg). `printed_form_text`
    означає «кандидат цілком складається з друкованих рядків бланка», і як
    доказ порожнечі це правильно тоді, коли в слоті стоїть СКЕЛЕТ слота
    (незаповнена дата «"__" ______ 20__ р.»). Але той самий вердикт дає й
    ЛЕЙБЛ-ПРИМІТКА ІНШОГО поля: на фото рамка значення заходила на рамку свого
    лейбла на третину рядка, геометрія відкидала правильного кандидата, і на
    місце `position_and_workplace` ставало
    "(військове звання, прізвище ім'я по батькові)" -- примітка ПОПЕРЕДНЬОГО
    поля. Поле оголошувалось «доведено порожнім» при тому, що значення
    ("старшина роти, військова частина Ж3085") стояло в документі поруч.

    Хибний ДОКАЗ дорожчий за прогалину: рев'юер, побачивши «слот порожній»,
    підтвердить порожнечу й не піде дивитись у документ (той самий висновок,
    що в C-06). Тому дужкова примітка бланка доказом не є НІКОЛИ -- вона
    свідчить, що ми взяли не той блок, а не що слот порожній. Поле йде у
    звичайний фолбек, тобто повертається до чесного «не знаємо».

    Геометрію виправлено окремо (див. _geometric_candidate); ця перевірка --
    не дублювання, а запобіжник на КЛАС: будь-який інший промах, що привів до
    примітки, теж більше не стане доказом.
    """
    if reason not in PROVEN_EMPTY_REASONS:
        return False
    if reason != "printed_form_text" or candidate is None:
        return True
    lines = [ln for ln in str(candidate).split("\n") if ln.strip()]
    return not (lines and all(_is_printed_label_note(ln) for ln in lines))
        ...
        if mode == "rank_and_name_tokenized":
            ...
            if raw is None:
                ...
                if (value is None and empty_proof_allowed
                        and proves_empty(value_reason, raw)):
            ...
            # Кандидат до перевірки -- він потрібен `proves_empty`, щоб
            # відрізнити скелет порожнього слота від чужої лейбл-примітки.
            regex_candidate = value
            if value is not None:
                value, reason = validate_regex_value(field, value, printed)
            if (value is None and empty_proof_allowed
                    and proves_empty(reason, regex_candidate)):
