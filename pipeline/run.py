...
#: Нижня межа «у документі взагалі є текст». Менше -> запис `unresolved` з
#: причиною «збій розпізнавання», а не «усі поля відсутні»: інакше порожній
#: скан виглядав би як звичайний needs_review і губився серед них.
#:
#: ЧОМУ 20 (рев'ю 22.08.2026, C-08: доти це був літерал без жодного рядка
#: обґрунтування, поруч із порогами, у яких походження заміряне). Замір по
#: всьому репозиторію: найкоротший ТЕКСТ документа, з якого пайплайн витягує
#: факт, -- 875 символів (довідка_лікування_02.docx з holdout; серед
#: заповнених бланків leave/deployment мінімум ще вищий). Тобто будь-яка межа
#: від 1 до ~850 однаково не чіпає жоден наш документ, і 20 стоїть не «на
#: око», а на іншому боці зазору в 40 разів. Сенс саме такої малої межі: вона мусить відрізняти
#: «OCR не дав НІЧОГО» (0-2 блоки, кілька символів шуму) від «документ
#: прочитано», і не сміє відкидати короткий, але справжній документ --
#: рукописний рапорт на пів сторінки цілком може мати 200 символів.
...
def build_resources(cfg: dict, force_no_llm=False) -> dict:
    ...
    res = {
        ...
        "ocr": None,
        "store": None,
        # Сухий прогін: сховище лишається для ЧИТАННЯ (дедуплікація), запис
        # вимкнено (_persist). Див. C-09 у _persist.
        "read_only": False,
        "warnings": [],
    }
...
def blank_meta(**overrides) -> dict:
    ...
    meta = {
        ...
        "review_queue": None,
        "subject": {},
        # ДРУГА (і далі) особа документа, згрупована за `group:` схеми
        # (рев'ю 22.08.2026, A-02). Доти цей ключ build_record віддавав, а
        # оркестратор не згадував ЖОДНОГО разу -- тобто друга особа
        # витягувалась і губилась на межі двох модулів. Сьогодні дірка
        # латентна (жодна схема не оголошує `group:`, тож значення завжди {}),
        # і саме тому проведення ключа дешеве: коли схема групу оголосить,
        # значення вже буде у виході, а не зникне мовчки.
        # У БД друга особа все одно не поїде (people приймає одну особу на
        # документ) -- але рев'юер її побачить, і це вже не тиха втрата.
        "extra_subjects": {},
...
def process_file(path: str, res: dict, cfg: dict, force_template=None,
    ...
    if not text or len(text.strip()) < MIN_DOCUMENT_TEXT_CHARS:
    ...
    try:
        ...
        # Статус документа визначає ОСНОВНИЙ факт -- знайдений за позначкою
        # `is_primary`, а не за індексом 0 (C-08). Фолбек на facts[0] лишається
        # для запису, зібраного не build_record-ом (напр. старий збережений
        # meta), але сам вибір більше не тримається на порядку списку.
        primary_fact = next((f for f in record["facts"] if f.get("is_primary")),
                            (record["facts"] or [None])[0])
        confirmed = bool(primary_fact) and bool(primary_fact.get("confirmed"))
        ...
        meta = dict(
            ...
            facts_expected=True,
            extra_subjects=record.get("extra_subjects") or {},
            source_kind=source_kind,
            domain=ident["domain"],
...
def _persist(meta: dict, text: str, res: dict) -> None:
    store = res.get("store")
    # СХОВИЩЕ ДЛЯ ЧИТАННЯ І ДЛЯ ЗАПИСУ -- РІЗНІ РЕЧІ (рев'ю 22.08.2026, C-09).
    # `--dry-run` раніше реалізовувався як `res["store"] = None`, а
    # дедуплікація читається саме зі `store` -- тобто в сухому прогоні гілка
    # `duplicate` була недосяжна ЗА ПОБУДОВОЮ, і `--dry-run` мовчки означав ще
    # й `--reprocess`. Тепер сухий прогін вимикає лише ЗАПИС: `store`
    # лишається на місці й дедуплікація працює так само, як у робочому
    # прогоні, тобто сухий прогін показує те, що станеться насправді.
    if store is None or res.get("read_only"):
        return
