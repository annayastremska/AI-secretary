...
# ПРАВИЛО, А НЕ ЧИСЛО (аудит абляцією, 24.08.2026). Поріг дорівнює вазі
# заголовка не випадково -- це і є саме правило: «влучення в заголовок бланка
# САМЕ ПО СОБІ достатнє, збіг анкорів без заголовка -- ні». Доти воно було
# записане ЧОТИРМА незалежними п'ятірками (`TITLE_WEIGHT`, `DEFAULT_MIN_SCORE`
# і `min_score: 5` у двох схемах), і рівність між ними не перевіряло НІЩО:
# заміряно, що підняти `TITLE_WEIGHT` до 6 коштує два тести й НУЛЬ перевірок
# корпусу -- тобто правило можна було розірвати, не зачепивши жодної цифри.
#
# Тепер зв'язок виражений, а не повторений. Схема все ще може оголосити свій
# `min_score:` (два бланки це роблять), але дефолт більше не є окремим числом,
# яке треба тримати рівним у голові.
DEFAULT_MIN_SCORE = TITLE_WEIGHT
