...
# Нумерація статей -- СТРУКТУРНА ознака нормативного акта, не фраза. Тому
# живе в коді (це форма документа), а порог -- у довіднику
# (`article_pattern_min`): скільки різних «Стаття N» вважати сигналом.
# Заміряно 22.08.2026: 3+ статей мають 25 із 43 нормативних і 0 із 14
# бланків/квитків/посвідчень/демо-документів.
_ARTICLE_RE = re.compile(r"стаття\s+\d+")
...
def classify_domain_rules(text: str, domains: dict):
    ...
    for domain, phrases in (domains or {}).items():
        ...
        # ТРЕТІЙ шлях: ФОРМАЛЬНІ ОЗНАКИ АКТА, без залежності від довжини.
        #
        # Причина (Анна, 22.08.2026): `normative` -- не тема, а РІД тексту.
        # Закон «Про оборону України» говорить про техніку, наказ про зміни до
        # інструкції -- про штат; змагаючись балами з тематичними доменами,
        # вони й отримували `equipment` (бал 5) і `staffing` (бал 4). А з
        # тематичним доменом акту дістається ВИД СУБ'ЄКТА -- тобто в реєстрі
        # осідає фантомний об'єкт, рівно те, чого цей блок і мусив не
        # допустити.
        #
        # Обидва попередні шляхи від довжини залежали: заголовок мусив бути
        # відомим, а fallback вимагав 20000 символів. Тому наказ на 14363
        # символи й НД ТЗІ на 11968 не проходили, МАЮЧИ досить фраз -- довжина
        # відсікала те, що вже було впізнане.
        #
        # Маркери -- формальні (хто ухвалив, де опубліковано, як акт говорить
        # про себе, як його скасовують), тому не залежать від теми. ЗАМІРЯНО:
        # кожен окремо дає 0 збігів у 14 ненормативних файлах, максимальна сума
        # серед них -- 0. Порог 2 -- запас: за цифрами вистачило б 1, але
        # цитата закону всередині службового документа можлива, і другий маркер
        # її відсіює.
        markers = _count_phrase_hits(low, phrases.get("formal_markers", []))
        article_min = phrases.get("article_pattern_min")
        if article_min and len(_ARTICLE_RE.findall(low)) >= article_min:
            markers += 1
        min_markers = phrases.get("min_formal_markers")
        if min_markers and markers >= min_markers:
            return domain, {domain: f"procedural_formal_markers:{markers}"}
