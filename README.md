...
- `data/inbox/` — папка-приймач: файли, які пайплайн обробляє за запуском.
- `data/output/` — результат локального прогону (та сама схема ключів, що
  планується в MinIO: `documents/<domain>/<id>.md`).
...
Пайплайн працює й **без моделі** (`--no-llm`): детермінований прохід дає
результат, а поля, які він не закрив, чесно позначені як прогалини — але це
неповний режим, `extraction: llm` поля лишаються порожніми. LLM і OCR
підключаються окремо й незалежно одне від одного.
