...
def rules_route(question):
    ...
    # нормативних документів» -- при 44 таких документах. Причина була не в
    # пошуку, а в тому, що питання ПРО КОРПУС нікуди не маршрутизувалось.
    #
    # Гейт вузький навмисно: «покривають/покриття/є в базі/склад бази» ПЛЮС
    # слово про базу або документи. «Який період відпустки у Ґоляша» під це не
    # підпадає -- там ідеться про особу, а не про корпус.
