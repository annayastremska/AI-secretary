...
#: Слова, через які фраза перестає бути ЛИШЕ привітанням: далі йде питання.
_HAS_A_QUESTION = re.compile(
    r"скільк|хто|що\s|який|яка|яке|які|де\s|коли|покажи|назв|список|перелік"
    r"|документ|відпуст|відрядж|черг|факт|№", re.I)


def is_greeting(question):
    """-> True, якщо фраза є привітанням/подякою і НІЧОГО не питає.
