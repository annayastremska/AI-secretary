...
#: ПРИВІТАННЯ -- правилом, а не моделлю.
#:
#: «Доброго ранку!» модель клала в `person_status`: у питанні немає ні дати, ні
#: підрахунку, зате є звертання -- і найближчим шаблоном для неї виявився
#: «що відомо про людину». Відповідь виходила «не знайшла такої особи», тобто
#: на привітання система відповідала відмовою про когось.
#:
#: Правило дешеве й безпечне саме тому, що воно ЗАКРИТЕ: збіг мусить бути на
#: усе питання цілком (`fullmatch` нижче), тому «привіт, скільки людей у
#: відпустці?» сюди не попаде -- там є про що відповідати.
_GREETING = re.compile(
    r"(?:добр\w+\s+(?:ранок|ранку|день|дня|вечір|вечора|доба|доби)"
    r"|доброго\s+здоров\W*я|бажаю\s+здоров\W*я"
    r"|привіт\w*|вітаю|салют|здоров"
    r"|дяку\w*|спасибі|подяка"
    r"|до\s+побачення|бувай\w*|на\s+все\s+добре|гарного\s+дня)",
    re.I)

#: РОЗМОВНІ ФРАЗИ, що не є привітанням: офтоп і питання про саму систему.
#:
#: Додані після заміру `measure_router --production-view`: прилад показав, що
#: «Розкажи анекдот» і «Розкажи щось цікаве» правила ведуть у `person_status`,
#: тобто на прохання пожартувати чат відповідає «не знайшла такої особи».
#: Перелік ЗАКРИТИЙ і складається з форм, у яких немає про що питати базу.
_CHITCHAT = re.compile(
    r"^(?:"
    r"розкажи\s+(?:анекдот|жарт|щось\s+цікаве|про\s+себе)"
    r"|як\s+(?:ти|справи|життя|воно)"
    r"|ти\s+(?:бот|людина|хто)\b.*"
    r"|хто\s+ти(?:\s+і\s+що\s+ти\s+вмієш)?"
    r"|що\s+ти\s+(?:вмієш|можеш)"
    r"|є\s+хтось\s+живий"
    r")\W*$", re.I)

#: Слова, через які фраза перестає бути ЛИШЕ розмовною: далі йде питання.
#:
#: МЕЖІ СЛІВ тут обов'язкові, і це не причепка. Перша версія була без них, і
#: `хто` збігалося всередині `хтось`: через це «привіт, є хтось живий?»
#: пролітало правило й ішло в модель. Знайшов прилад, не тест -- тест перевіряв
#: ті фрази, які я сама й придумала.
_HAS_A_QUESTION = re.compile(
    r"\bскільк|\bхто\b|\bщо\s|\bякий\b|\bяка\b|\bяке\b|\bякі\b|\bде\s|\bколи\b"
    r"|\bпокажи|\bназв|\bсписок\b|\bперелік\b"
    r"|документ|відпуст|відрядж|черг|факт|№", re.I)


def is_greeting(question):
    """-> True, якщо фраза розмовна (привітання, подяка, офтоп) і НІЧОГО не
    питає про дані.

    Друга умова тут головна. Без неї «Привіт! скільки людей у відпустці?»
    отримало б розмовну відповідь замість числа -- тобто правило зламало б
    рівно ті питання, задля яких система існує.

    Порядок перевірок теж має значення: закритий перелік розмовних форм
    (`_CHITCHAT`) стоїть ПЕРЕД гейтом питальних слів, бо в самих цих формах
    питальні слова й живуть («хто ти», «що ти вмієш»). Інакше гейт з'їв би те,
    задля чого перелік і написаний.
    """
    ...
    if _CHITCHAT.match(text):
        return True
    if _HAS_A_QUESTION.search(text):
        return False
    return bool(_GREETING.search(text))


# ── Населений пункт ─────────────────────────────────────────────────────────
#
# Розбір звернення 05259c (Аня, 29.08). Дві репліки: «Покажи документ №102» ->
# «але вони саме в житомирі?». Друга отримала впевнену відповідь ПРО ІНШЕ, бо
# зрізу по місцю в каталозі не існувало: серед параметрів були дата, вимір,
# номер документа, ПІБ і підрозділ -- і жодного місця.
#
# ГОЛОВНЕ РІШЕННЯ ТУТ: кандидат на назву пункту НЕ приймається на віру. Він
# зіставляється зі значеннями, які СПРАВДІ лежать у базі, і у відповідь іде
# назва з бази, а не з питання. Причина проста: правило, що хапає будь-яке
# слово після «у/в», почне бачити місто у «у відпустці» й «у черзі», і зробить
# це тихо -- відповідь буде виглядати правильною.
#
# Другий бік того самого: назва СХОЖА, але в даних відсутня («у Жмеринці») --
# це відмова, а не найближчий збіг. Те саме правило, що для номерів
# документів: не виправляємо і схожих не підставляємо.

#: Мінімальна довжина кореня для збігу. Три літери дають хибні влучення на
#: довгих назвах («мал» -> «м. Малий Ясенець»), і людина цього не побачить.
PLACE_MIN_ROOT = 4

#: Слова, які стоять після «у/в», але місцем не є НІКОЛИ. Перелік закритий і
#: короткий навмисно: він не «фільтр сміття», а захист від найчастіших
#: сусідів -- решту відсіює звірка з базою.
_NOT_A_PLACE = {
    "відпустці", "відпустку", "відпустка", "відрядженні", "відрядження",
    "черзі", "базі", "частині", "роті", "взводі", "управлінні", "штатці",
    "списку", "переліку", "документі", "наказі", "запиті", "чаті",
}

#: «у Житомирі», «в Сухоброді», «до Кривоярська». Місто може стояти й без
#: прийменника («хто ще Житомир»), але такого в мові не буває -- тому шукаємо
#: саме з прийменником: це і є ознака, що слово вживається як місце.
_PLACE_AFTER = re.compile(
    r"\b(?:у|в|до|біля|при)\s+((?:м\.|с\.|смт)?\s*[А-ЯІЇЄҐа-яіїєґ][А-ЯІЇЄҐа-яіїєґ'’\-]{2,})",
    re.U)


def known_places():
    """Значення місць із бази: відпустки й відрядження разом. -> перелік.

    Читання, кешу немає навмисно: перелік маленький, а застарілий кеш тут
    означав би відмову на пункт, який щойно з'явився в документі.
    """
    try:
        rows = _run_template_sql(
            "SELECT DISTINCT btrim(f.value) AS v "
            "FROM facts f JOIN dimensions d ON d.id = f.dimension_id "
            "WHERE d.code = ANY(%(dims)s) AND f.value IS NOT NULL "
            "  AND btrim(f.value) <> '' ",
            {"dims": PLACE_DIMS})
    except Exception:                               # noqa: BLE001
        # База недоступна -> пунктів ми не знаємо, і це НЕ причина валити всю
        # дорогу правил. Порожній перелік означає «жоден пункт не впізнано»,
        # тобто питання поїде далі -- як і до появи цього зрізу.
        return []
    out = []
    for r in rows or []:
        # Ключ читаємо обережно: `_run_template_sql` підмінюється заглушкою і в
        # тестах, і в приладі `measure_router` (`rules_covered`), а заглушка
        # віддає свій рядок. Перша версія брала `r["v"]` і валила KeyError --
        # тобто моя нова функція ламала ЧУЖИЙ тест дороги доказів, не маючи до
        # неї стосунку.
...
#: Виміри, у яких лежить місце. Відпустка й відрядження РАЗОМ: людину цікавить
#: «хто ще там», а не «хто там у відпустці».
PLACE_DIMS = ["leave_place", "deployment_location"]


def _place_lookup(stem):
    """Корінь -> назва пункту з бази, або None."""
    stem = stem.lower()
    for value in known_places():
        bare = re.sub(r"^(?:м\.|с\.|смт)\s*", "", value).lower()
        if bare.startswith(stem) or value.lower().startswith(stem):
            return value
    return None
...
#: Слова, які роблять коротку репліку питанням НЕ про людей у пункті.
#: Закритий перелік: якщо в короткій фразі є документ, норма чи черга, то місто
#: в ній -- обставина, а не предмет питання.
#: `черз` окремо від `черг` -- це не дублікат: в українській тут чергування
#: г->з («черга», але «в черзі»), і стем «черг» у слові «черзі» не збігається.
#: Зловив мій же тест на фразі «що в черзі у житомирі».
_SHORT_NOT_ABOUT_PEOPLE = re.compile(
    r"документ|наказ|норм|стаття|пункт|черг|черз|факт|тем|цитат|№", re.I)


def is_bare_place_followup(question, lookup=None):
    """-> True, якщо репліка це КОРОТКЕ «а у Житомирі?» без слова «хто».

    Знайдено Анею 29.08 на живому діалозі: «хто зараз у рівному» відповіло
    правильно (Рівного в даних немає -- відмова), а наступна репліка «а у
    житомирі» впала у відмову «питання не лягає на жодну дорогу». Причина в
    моєму ж гейті: він вимагав слова «хто», «скільки» або «там», а в цій
    репліці немає жодного -- вона складається з прийменника й назви міста.

    Так люди й питають: попереднє питання лишається в силі, міняється лише
    один параметр. Тому умова тут не «є питальне слово», а «репліка КОРОТКА,
    у ній є відомий пункт і немає нічого іншого, про що можна питати».

    Довжина обмежена п'ятьма словами навмисно: у довгій фразі без питального
    слова місто майже завжди обставина («у житомирі підписали наказ»), і
    хапати таке правилом означало б відповідати не на те питання.
    """
    text = (question or "").strip()
    words = [w for w in re.split(r"\s+", text) if w]
    if not words or len(words) > 5:
        return False
    if _SHORT_NOT_ABOUT_PEOPLE.search(text):
        return False
    if extract_place(text, lookup=lookup) is not None:
        return True
    # НАЗВА БЕЗ ПРИЙМЕННИКА: «а Тихолісся?». Загальний `extract_place` вимагає
    # прийменника навмисно (це і є ознака, що слово вживається як місце), але в
    # КОРОТКІЙ репліці-продовженні прийменника часто немає взагалі -- людина
    # називає лише те, що змінилось.
    #
    # Послаблення безпечне рівно тому, що збіг однаково перевіряється базою:
    # слово, якого немає серед місць у документах, сюди не пройде. Тому
    # відсіюємо лише службові слова-початки.
    look = lookup or _place_lookup
    for w in words:
        w = w.strip("?!.,;:«»\"'()").strip()
        if len(w) < PLACE_MIN_ROOT or w.lower() in _NOT_A_PLACE:
            continue
        if w.lower() in ("тоді", "чому", "коли", "хіба", "може", "тобто"):
            continue
        for cut in range(0, 4):
            stem = w[:len(w) - cut]
            if len(stem) < PLACE_MIN_ROOT:
                break
            if look(stem):
                return True
    return False
...
def extract_place(question, lookup=None):
    """-> назва населеного пункту ТАК, ЯК ВОНА В БАЗІ, або None.

    `lookup` -- підміна звернення в базу для тестів. У проді не передається.

    Відмінок знімається обрізанням хвоста, а не морфологією: беремо корінь і
    питаємо базу, чи є значення, яке з нього починається. Тобто здогад завжди
    перевіряється даними -- і саме тому «у Жмеринці» дає None, а не «м.
    Житомир».
    """
    look = lookup or _place_lookup
    text = question or ""
    #: Підрозділ важливіший: «у 2 роті» -- не населений пункт. Перевіряємо
    #: ДО пошуку місця, інакше «роті» пішло б у звірку з базою.
    if _SUBDIVISION.search(text.lower()):
        return None
    for m in _PLACE_AFTER.finditer(text):
        word = re.sub(r"^(?:м\.|с\.|смт)\s*", "", m.group(1)).strip()
        if word.lower() in _NOT_A_PLACE:
            continue
        #: Обрізаємо до чотирьох варіантів кореня: слово цілком, потім по
        #: одній літері з хвоста. Далі не йдемо -- корінь коротший за
        #: PLACE_MIN_ROOT дає хибні збіги.
        for cut in range(0, 4):
...
#: Місяць словом -- ознака ЯВНОЇ дати в питанні (цифр у ньому може не бути).
_MONTH_WORD = re.compile(
    r"січн|лют|берез|квітн|трав|черв|липн|серпн|верес|жовтн|листопад|грудн",
    re.I)


def unknown_place_candidate(question, lookup=None):
    """-> слово, яке ВИГЛЯДАЄ назвою пункту, але в даних його немає. Або None.

    Нащо окремо від `extract_place`. «хто зараз у рівному» -- Рівного серед
    місць у документах немає, тому `extract_place` чесно віддає None, і правила
    не мають маршруту. Питання падає в ЗАГАЛЬНУ відмову («не лягає на жодну
    дорогу»), хоча правильна відмова інша й вона в каталозі є: «такого пункту
    серед місць немає, нуль означав би, що там нікого немає».

    У живому прогоні Ані правильну відмову обрав ВЕКТОРНИЙ ярус -- за схожістю
    з прикладом «Хто у Жмеринці?». Тобто вона з'являлась, поки питання
    випадково було схоже на приклад; на «у рівному» схожості не вистачило.
    Покладатись на це не можна: правильність відповіді не мусить залежати від
    того, наскільки формулювання нагадує приклад.

    МЕЖІ тут суворі, бо ціна помилки -- украдене питання. Кандидат приймається
    лише коли в питанні НЕМАЄ нічого іншого, про що можна відповісти: ні стану,
    ні дати, ні номера документа, ні підрозділу, ні маркерів нормативки.
    Інакше «хто у відпустці в травні» пішло б у відмову про населений пункт.
    """
    text = (question or "").strip()
    if not text or not _ASKS_WHO_OR_WHERE.search(text.lower()):
        return None
    #: Будь-яка інша сутність у питанні -> це не питання про пункт.
    ...
    if _SHORT_NOT_ABOUT_PEOPLE.search(text):
        return None
    # ДАТА блокує тільки ЯВНА. «хто зараз у рівному» -- `extract_dates` віддає
    # тут «сьогодні», бо в питанні є слово «зараз», і перша версія цього
    # запобіжника через це вбивала саме той випадок, задля якого написана.
    # «Зараз» у питанні про пункт -- норма мови, а не дата: людина не звужує
    # період, вона питає про поточний стан.
    if re.search(r"\d", text) or _MONTH_WORD.search(text):
        return None
    if extract_place(text, lookup=lookup) is not None:
        return None          # пункт відомий -- це не наш випадок
    for m in _PLACE_AFTER.finditer(text):
        word = re.sub(r"^(?:м\.|с\.|смт)\s*", "", m.group(1)).strip()
        if word.lower() in _NOT_A_PLACE or len(word) < PLACE_MIN_ROOT:
            continue
        return word
    return None
...
def rules_route(question):
    ...
    # Привітання -- одразу після гейта дати й до всього підрахункового: воно
    # не має ні дати, ні числа, і будь-яке правило нижче схопить його випадково.
    ...
    # ЗАПИС У БАЗІ -- окреме правило, і СТОЇТЬ ВИЩЕ за номер документа.
    #
    # Ми самі показуємо під відповіддю «документ №207 (запис №33 у базі)», а
    # спитати про запис було неможливо: `extract_doc_number` хапав 33 як номер
    # на папері й картка виходила про ІНШИЙ документ. Тобто система показувала
    # ідентифікатор, яким не можна скористатись, а спроба -- давала впевнену
    # відповідь не про те.
    #
    # Особливо це важливо там, де номера на папері немає взагалі (58 із 205
    # документів): запис -- єдиний спосіб на такий документ послатись.
    m_rec = re.search(r"запис\w*\s*№?\s*(\d{1,6})", low)
    if m_rec:
        return "doc_by_record", {"record_id": m_rec.group(1)}
    ...
    # теми вони».
    # НАСЕЛЕНИЙ ПУНКТ -- правилом, і саме тут: після номера документа й черги
    # (там свої однозначні маркери), але ПЕРЕД підрахунками стану. Питання
    # «хто ще в Житомирі» містить слова відсутності не завжди, а от підрахунок
    # стану схопить його охоче -- і відповість по всій частині.
    #
    # Гейт дешевий: `extract_place` спершу перевіряє, що це не підрозділ, і
    # лише потім звіряється з базою. Порожній результат означає «місця в
    # питанні немає», тобто правило нічого не забирає.
    if _ASKS_WHO_OR_WHERE.search(low) or is_bare_place_followup(question):
        place = extract_place(question)
        if place:
            return "list_by_place", {"place": place}
        # Схоже на назву пункту, але в даних його немає -> ТА САМА відмова, що
        # її дає каталог. Без цього питання йшло в загальну відмову «не лягає
        # на жодну дорогу», і правильність залежала від того, чи випадково
        # формулювання схоже на приклад у каталозі (див. розбір у
        # `unknown_place_candidate`).
        if unknown_place_candidate(question):
...
def model_route(question, history=None):
    ...
    if "place" in need and "place" not in params:
        # Місце беремо з ПИТАННЯ, а не з поля моделі: у схемі відповіді моделі
        # поля `place` немає (додавати його -- окрема зміна схеми й окремий
        # замір). Правила тут сильніші за модель саме тому, що вони звіряються
        # з базою: `extract_place` повертає лише те, що в даних справді є.
        place = extract_place(question)
        if not place:
            # Слово, схоже на назву пункту, у питанні було (інакше ми б сюди
            # не потрапили), але в даних його немає -- це чесна відмова.
            # Найближчу назву НЕ підставляємо: це був би перелік про інше
            # місце, і людина цього не побачила б.
            return "place_unknown", {}
        params["place"] = place
...
#: питання».
#: `place` -- додано 29.08 разом із `list_by_place`. І повторилось ТОЧНО те, про
#: що попереджає абзац вище: без цього рядка запит падав із «query parameter
#: missing: place», виняток глушився в `_catalog_tier`, і питання «хто ще в
#: житомирі» тихо їхало у відмову -- при тому, що правило впізнавало його
#: правильно й шаблон існував. Тобто пропущений рядок тут знову виглядав як
#: «чат не розуміє питання». Тому нижче стоїть тест, який звіряє цей перелік із
#: параметрами каталогу, щоб третього разу не було.
PLACE_DIM_LABEL = {"leave_place": "відпустка",
                   "deployment_location": "відрядження"}

_SQL_PARAM_NAMES = ("dims", "on_date", "date_from", "date_to", "name_pattern",
                    "doc_number", "query", "subdivision", "place")
...
def run_template(template_id, params):
    ...
    if template_id == "count_by_state_on_date":
        ...
        # на екрані означають, що система не доробила відповідь.
        #
        # Назва пункту -- ІЗ БАЗИ (`params["place"]`), не з питання: у питанні
        ...
        if not rows:
            ...
            # було б неправдою.
            lines.append(f"{_people(len({r['name'] for r in rows}))} за "
                         f"документами у {_esc(place)}:")
            for r in rows[:RAW_ROWS_SHOWN]:
                kind = PLACE_DIM_LABEL.get(r.get("dim"), "відсутність")
                lines.append(f"- {_esc(r['name'])} — {kind}, "
                             f"{_fmt_period(r)} ({_doc_ref(r)})")
            if len(rows) > RAW_ROWS_SHOWN:
                lines.append(f"…ще записів: {len(rows) - RAW_ROWS_SHOWN} "
                             "(не показано, щоб відповідь лишалась читною)")
        # ЗРІЗ БАЗИ -- обов'язковий у кожній відповіді (правило продукту).
        #
        # Зловив `test_every_answer_states_the_as_of_date[list_by_place]`, і
        # зловив по праву: рядок про зріз дописував ЗАГАЛЬНИЙ табличний рендер,
        # а я замінила його своєю гілкою -- і разом із машинними колонками
        # прибрала те, що мусило лишитись. Тобто «покращення подачі» тихо
        # зняло правило продукту.
        #
        # Дати тут немає в параметрах: зріз по місцю не має «на дату», він
        # показує всі записи. Тому зріз -- це стан БАЗИ на сьогодні, і сказано
        # саме так, а не «на дату».
        lines.append(f"Зріз: стан бази на {datetime.date.today()}.")
        if u_rows:
            lines.append("Окремо непідтверджені (чекають перевірки людиною, у "
                         f"підсумок не входять): {len(u_rows)}")
        else:
            lines.append("Непідтверджених записів по цьому пункту: 0.")
...
#: ─── ДВІ ПЕРЕВІРКИ ПРОТИ ВПЕВНЕНОЇ ДУРНИЦІ (29.08) ────────────────────────
#:
#: Знайдено Анею живцем: на питання «скільки набоїв на складі» ярус вільного
#: SQL склав запит по виміру `document_number` і віддав 129 НОМЕРІВ ДОКУМЕНТІВ
#: як відповідь. Ні слова, що про набої в базі немає нічого.
#:
#: Це найгірший клас відмови -- впевнена відповідь не про те. Валідатор тут
#: не винен: він міряє БЕЗПЕКУ (один SELECT, allowlist, LIMIT), і в його
#: шапці прямо написано, що доречність він не міряє. Показ SQL на екрані --
#: рейка для нас, але не для гостя, який SQL не читає.
#:
#: Обидві перевірки МЕХАНІЧНІ (правило Ані: не судити моделью модель).

#: Слова, які в питанні є службовими й предметом бути не можуть. Перелік
#: закритий і короткий: він не «фільтр сміття», решту вирішує звірка з базою.
_NOT_A_SUBJECT = {
    "скільки", "кількість", "яка", "який", "яке", "які", "яких", "яким",
    "хто", "кого", "кому", "чому", "коли", "куди", "звідки", "де",
    "покажи", "дай", "назви", "перелік", "список", "зараз", "сьогодні",
    "будь", "ласка", "мені", "нам", "все", "всі", "усі", "цього", "цей",
    "база", "базі", "бази", "даних", "система", "чаті", "чат",
}

#: Питання про КІЛЬКІСТЬ. Саме на такому й прорвало.
_QUANTITY_ASK = re.compile(r"\bскільк\w*|\bкількіст\w*", re.I)

#: Агрегат у SQL. Питання «скільки» без агрегату -> запит віддасть перелік,
#: тобто відповість не на те, що спитали.
_SQL_AGGREGATE = re.compile(r"\b(count|sum|avg)\s*\(", re.I)

SUBJECT_MIN_ROOT = 5


def _subject_known_to_db(root):
    """Чи знає база хоч щось про це слово. -> True/False.

    Три місця, і кожне -- окреме питання:
      назви вимірів  -- чи є ТАКА ВЕЛИЧИНА (відпустка, звання, тривалість);
      значення фактів -- чи є ТАКЕ ЗНАЧЕННЯ (Житомир, майор, 1123);
      назви об'єктів  -- чи є ТАКИЙ ОБ'ЄКТ (прізвище).

    База недоступна -> вважаємо, що знає: інакше збій бази перетворився б на
    «у базі такого немає», а це вже брехня іншого роду.
    """
    # Пошук по ВХОДЖЕННЮ, не по початку рядка. Перша версія шукала «житом%» і
    # не бачила значення «м. Житомир» -- тобто мій же захист відмовив би на
    # питанні про місто. Знайдено живим прогоном проти бази, тестами зі
    # заглушкою це не ловилось.
    pat = "%" + root + "%"
    try:
        rows = _run_template_sql(
            "SELECT 1 AS hit FROM dimensions "
            " WHERE lower(name) LIKE %(pat)s OR lower(code) LIKE %(pat)s "
            " LIMIT 1", {"pat": pat})
        if rows:
            return True
        rows = _run_template_sql(
            "SELECT 1 AS hit FROM facts "
            " WHERE lower(btrim(value)) LIKE %(pat)s LIMIT 1", {"pat": pat})
        if rows:
            return True
        rows = _run_template_sql(
            "SELECT 1 AS hit FROM objects "
            " WHERE lower(canonical_name) LIKE %(pat)s LIMIT 1", {"pat": pat})
        return bool(rows)
    except Exception:                               # noqa: BLE001
        return True


def unknown_subject(question, probe=None):
    """Слово, про яке база не знає НІЧОГО, або None. -> str|None.

    Дивимось лише на слова від SUBJECT_MIN_ROOT літер: коротші в українській
    дають хибні збіги коренем (це вже було виміряно на місцях, PLACE_MIN_ROOT).
    Досить ОДНОГО впізнаного слова, щоб питання пішло далі: мета -- відсікти
    предмет, якого в корпусі немає взагалі, а не вивіряти кожне слово.
    """
    look = probe or _subject_known_to_db
    words = [w for w in re.findall(r"[а-яіїєґa-z]+", question.lower())
             if len(w) >= SUBJECT_MIN_ROOT and w not in _NOT_A_SUBJECT]
    if not words:
        return None
    for w in words:
        if look(w[:SUBJECT_MIN_ROOT]):
            return None
    # Жодне слово не впізнане -> називаємо найдовше: воно найімовірніше й є
    # предметом питання.
...
def tier2_answer(question):
    ...
    refuse = ("Не знайшла відповіді: питання не лягає на жоден шаблон, а "
              ...
              "номер документа).")
    # ПЕРЕВІРКА 1: предмет питання відсутній у корпусі.
    # Робиться ДО моделі -- і тому, що відповідь мусить бути відмовою, і тому,
    # що це економить 30 с очікування на завідомо непотрібний запит.
    missing = unknown_subject(question)
    if missing:
        return (f"Не знайшла: у базі немає даних про «{_esc(missing)}». "
                f"Вона знає особовий склад, відпустки, відрядження й "
                f"документи цих подій — про інше даних у ній немає."), \
               ["нешаблонний запит не складався: предмет питання відсутній "
                "у базі", f"незнайоме слово: {missing}"]
    ...
    # ПЕРЕВІРКА 2: питання про кількість -- відповідь мусить бути числом.
    # Саме тут і прорвало: «скільки набоїв» дало SELECT f.value без агрегату,
    # тобто перелік значень. Перелік не є відповіддю на «скільки».
    if _QUANTITY_ASK.search(question) and not _SQL_AGGREGATE.search(sql):
        return ("Не знайшла: питання про кількість, а запит склався на "
                "перелік значень, не на підрахунок. Спробуйте перефразувати."), \
               ["нешаблонний запит відкинуто: питання про кількість, "
                "у запиті немає підрахунку (COUNT/SUM/AVG)", "SQL моделі:", sql]
    try:
        with _connect() as conn:
            with conn.cursor() as cur:
        ...
        # НАЗВА ВИНЯТКУ -- НЕ ДЛЯ ЛЮДИНИ. «ProgrammingError» не каже ні що
        # сталось, ні що робити далі: це наша кухня, і в СЛІДІ вона лишається.
        # На екран іде причина людською мовою й підказка, як переформулювати --
        # саме на цьому падали 2 з 8 формулювань одного питання (значення лежить
        # текстом, а порахувати можна лише число), і відмова тоді нічим не
        # допомагала.
        return ("Не знайшла: запит склався, але база його не виконала. "
                "Найчастіше це означає, що значення в документі лежить "
                "текстом, а порахувати його можна лише числом. Спробуйте "
                "спитати простіше: назвіть стан (відпустка / відрядження), "
                "дату або номер документа."), [
            "нешаблонний запит, помилка виконання",
            f"клас винятку: {type(exc).__name__}",
            f"текст бази: {str(exc).strip()[:200]}",
            "SQL:", sql]
