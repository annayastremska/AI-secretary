...
#: Ярус вільного SQL УВІМКНЕНИЙ лише явно. Рішення 25.08.2026, зафіксоване в
#: docs/tasks/2026-08-25_stage4-progress.md і в критеріях приймання (П7).
#:
#: Причина не в безпеці -- безпека тут якраз пророблена: рівно один SELECT,
#: без DML/DDL, read-only і на рівні DSN, і на рівні сесії, примусовий
#: LIMIT 200, таймаут 5 с. Проблема в іншому: валідатор перевіряє, що запит
#: БЕЗПЕЧНИЙ, і не перевіряє, що він відповідає на ПОСТАВЛЕНЕ питання. Тобто
#: ярус може віддати впевнену цифру не на те питання -- а це рівно те, що
#: CLAUDE.md забороняє: «відмова краща за вигадку», «цифра без джерела гірша
#: за відсутність відповіді».
#:
#: Вимкнений ярус означає, що нешаблонне питання доходить до чесної відмови
#: (ANSWER_REFUSE) із підказкою, як перефразувати. Це передбачена ціна
#: правила, не регресія.
#:
#: Умова перегляду: з'явиться прилад, який міряє СЕМАНТИЧНУ правильність
#: згенерованого SQL (тест-сет питань з еталонними цифрами) -- тоді вмикати з
#: цифрою в руках, а не на віру. Вмикається `CHAT_FREE_SQL=1`.
FREE_SQL_ENABLED = os.environ.get("CHAT_FREE_SQL", "0").strip().lower() not in (
    "", "0", "false", "no", "off")
