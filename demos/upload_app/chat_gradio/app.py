        ...
        with gr.Row(equal_height=False):
            with gr.Column(scale=0, min_width=272, elem_id="sidebar"):
                ...
                if guest_qr_available():
                    ...
                    # Підпис кнопки -- назва ДРУГОГО коду, того, на який вона
                    # перемикає. Усе про пам'ятку -- англійською (Аня 30.08):
                    # сама пам'ятка англійська, і підпис іншою мовою обіцяв би
                    # не те, що людина побачить після сканування.
                    swap = ('<button type="button" id="qr-swap" '
                            'aria-controls="guest-qr">Switch to English '
                            'guide</button>') if both else ""
                    gr.HTML(
                        ...
                        + ('<img src="/static/qr-jury.png" '
                           'alt="QR code of the jury guide" '
                ...
                # НАЗВА МОДЕЛІ З ПАНЕЛІ ПРИБРАНА (Аня 30.08): після другого QR
                # і перемикача місця не лишилось.
                #
                # Прибрана саме НАЗВА, а не рядок стану. Різниця важлива:
                # «MamayLM 27B» -- це відповідь на питання «що там усередині»,
                # і його ставлять голосом, а не читають із панелі. А
                # «модель недоступна — працюють правила» -- це попередження
                # про ІНШУ роботу системи: відповіді стають коротшими й
                # вибираються правилами, і людина мусить бачити причину.
                # Тому напис лишається рівно тоді, коли він щось означає.
                state_text = ("" if model_available()
                ...
                # Порожній блок не малюємо взагалі: інакше в панелі лишиться
                # відступ від нічого -- і це читається як недомальований екран.
                if state_text:
