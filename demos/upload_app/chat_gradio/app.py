        ...
        with gr.Row(equal_height=False):
            ...
            with gr.Column(scale=1, elem_id="main-col"):
                ...
                with gr.Column(elem_id="hero", visible=True) as hero:
                    ...
                    # Рядки по три -- циклом, а не двома літералами `[:3]` і
                    # `[3:]`. Кнопок стало дев'ять (три питання по нормативці),
                    # і другий літерал зібрав би шість в один рядок. Цикл
                    # тримає розкладку при будь-якій кількості.
                    hero_btns = []
                    for _start in range(0, len(EXAMPLES), 3):
                        with gr.Row(elem_id="hero-cards"):
                            hero_btns += [gr.Button(q)
                                          for q in EXAMPLES[_start:_start + 3]]
