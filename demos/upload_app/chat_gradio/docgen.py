...
def done_line(state):
    """Рядок із номером звернення. -> «звернення: xxxxxx».

    ДВОКРАПКА ТУТ ОБОВʼЯЗКОВА, і це не оформлення. `render_reply` виносить
    номер у дрібний підпис знизу, а з тіла відповіді вирізає його регуляркою,
    де двокрапка стоїть у шаблоні. Без неї номер лишався в тексті І
    додавався підписом -- Аня побачила його двічі в одному повідомленні.

    Тобто формат рядка тут -- частина стику з рендером, а не смак.
    """
...
def step(state, text):
    ...
    if st["kind"] is None:
        ...
        if not kind:
            ...
            if not raw:
                _log(st, "started")
                # Порожній рядок перед номером -- вимога стику з рендером:
                # він вирізає з тіла саме «\n\nзвернення: …».
                return st, ask + "\n\n" + done_line(st), None
            return st, ask, None
        st["kind"] = kind
        st["idx"] = 0
