...
def _lexical_kw(su):
    """Аргументи до лексичного пошуку: розширення синонімами -- ВИКЛЮЧЕНО.

    Замір Андрія 27.08 («Негативні результати -- не повторювати»): дописування
    синонімів у текст запиту шкодить. Той самий розширений текст іде і у фільтр
    `tsv @@`, і в `ts_rank`, а ранг винагороджує за збіги ІЗ ЗАПИТОМ -- тому
    додані слова обганяють оригінальне розрізнювальне. Правильна форма --
    окрема гілка для RRF, і її поки немає.

    Сьогодні розширення й так не відбувається: таблиця синонімів лежить в
    експериментальній схемі, якої роль читання не бачить. Але це означає, що
    безпечний стан тримає КОНФІГУРАЦІЯ, а не код: досить дати доступ або
    перекласти таблицю в постійну схему -- і зміряно-шкідлива поведінка тихо
    ввімкнеться саме тут. Тому просимо явно.

    Стара версія модуля пошуку аргумента не має, тому передаємо лише за
    наявності в підписі: інакше був би TypeError на кожному запиті.
    """
    try:
        import inspect
        if "synonyms" in inspect.signature(su.lexical).parameters:
...
def answer(question):
    ...
    with psycopg.connect(_t._dsn(), autocommit=True) as conn,             conn.cursor() as cur:
        fused = su.dedupe_by_text(
            cur, su.rrf_merge(su.lexical(cur, question, **_lexical_kw(su)),
