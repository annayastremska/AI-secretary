...
def summary(path=None, since=None):
    """Зведення по всіх ходах -- те, для чого слід і потрібен машині."""
    rows = read_all(path)
    if not rows:
        return {"turns": 0}
    # ВІКНО ЗА ЧАСОМ, і чому воно тут з'явилось.
    #
    # 28.08 після прогону набору на сервері `--check` сказав «ЗЛАМАНО: 2 без
    # джерела, 4 збої». Усі шість записів були ТЕСТОВІ: набір писав у той самий
    # `logs/chat-trace.jsonl`, який читає прилад, а тести навмисно перевіряють
    # відповідь «база недоступна» (підставні реквізити -> OperationalError) і
    # відповідь без блоку джерела. Прилад міряв власний набір.
    #
    # Причину виправлено в `tests/conftest.py`: слід тестів іде в окремий файл.
    # А ці шість записів у файлі лишаються -- історію не переписуємо, бо в
    # ньому лежать номери звернень, за якими розбираються скарги (перелік
    # Дениса). Тому вікно: `--since` дозволяє порахувати правило на ходах ПІСЛЯ
    # правки, і обидва числа називаються вголос.
    #
    # Спроба відсіяти тестові записи за відсутністю номера звернення НЕ
    # спрацювала: вони його мають (ішли через `answer()`). Перша версія фільтра
    # брала ще й неіснуюче поле `cid` і відкинула ВЕСЬ файл -- «слідів нуль».
    # Обидва провали зловлені тим, що прилад друкує, скільком відкинув.
    skipped = 0
    if since:
        before = len(rows)
        ...
        if not rows:
            return {"turns": 0, "skipped_before_since": skipped}
    roads, templates = {}, {}
    ...
    return {
        ...
        #: Скільком ходів відкинуто вікном `--since`. Друкується приладом:
        #: звужена вибірка мусить бути ВИДНОЮ, інакше «OK» нічого не означає.
