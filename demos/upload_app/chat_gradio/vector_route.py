...
def catalog_routes(path=CATALOG_PATH):
    ...
    Шаблон із `vector_route: false` пропускається. Це не оптимізація, а межа
    методу: векторний ярус обирає шаблон за СХОЖІСТЮ ФОРМУЛЮВАННЯ, а деякі
    шаблони обираються не за формулюванням, а за станом ДАНИХ.

    Приклад, через який поле й з'явилось (27.08): `date_invalid` -- відмова на
    неіснуючу дату. Його приклад «Скільком осіб у відпустці 2026-02-30?»
    майже дослівно збігається зі звичайним питанням «…2026-10-10?»: різниця в
    двох цифрах, і жодна міра схожості її не побачить. Тобто цей шаблон у
    векторному індексі не міг би допомогти, зате міг би віддати ВПЕВНЕНО
    НЕПРАВИЛЬНИЙ маршрут -- відмову на дійсну дату. А впевнено-неправильний
    маршрут гірший за фолбек: фолбек усе одно дасть правильну відповідь.

    Такі шаблони обирають правила, які вміють перевірити саму умову
    (`impossible_dates`), а не вгадати її з тексту.
    """
    with open(path, encoding="utf-8") as f:
        templates = yaml.safe_load(f)["templates"]
    return [(t["id"], list(t.get("examples") or [])) for t in templates
            if t.get("examples") and t.get("vector_route") is not False]
