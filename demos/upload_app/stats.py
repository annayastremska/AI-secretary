...
def _rows_to_map(rows, key):
    """[{key: 'leave', 'n': 3}] -> {'leave': 3}.

    Порожній ключ -> «тип не визначено», а не «(не вказано)». Ховати його
    не можна -- це теж стан даних, але дужки з порожнім словом у рядку
    «відпустка 95 · відрядження 64 · (не вказано) 1» читаються як недоробка
    сторінки, а не як факт про документ. Тепер сказано, ЧОГО саме немає.
    """
    out = {}
    for row in rows:
        name = row.get(key)
        out["тип не визначено" if name in (None, "") else str(name)] = int(row["n"])
    return out
...
#: КОРОТКО, і це вимога Ані 28.08 («тексту трохи забагато, одне й те саме
#: повторюється двічі»). Визначення перетину стоїть на сторінці РІВНО
#: один раз -- тут, під числом, бо цього вимагає правило «цифра без
#: джерела не показується». Примітка внизу розділу його більше не
#: переказує: там лишилось тільки те, чого визначення не покриває.
CONFLICTS_HOW = ("накладання періодів двох відсутностей однієї особи; "
                 "запит по базі при відкритті сторінки")
