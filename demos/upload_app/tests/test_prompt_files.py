...
def _route_system_before_the_move():
    ...
    lines.append("- вільний_sql: питання про дані бази, яке не лягає на "
                 "жоден шаблон")
    # ОНОВЛЕНО 28.08 разом із самим промптом, і навмисно ВРУЧНУ.
    #
    # Цей тест доводить, що переїзд промпта з коду у файл був побайтово
    # точним. Коли текст промпта міняється СВІДОМО, літерал тут мусить
    # оновитись тим самим рухом -- інакше тест перестає бути охоронцем і стає
    # шумом, який щоразу «просто оновлюють».
    #
    # Зміна: звужений пункт про відмову (зміряна правка Андрія -- нормативні
    # питання про строк і ліміт падали у відмову ще на маршрутизації: 5 із 17,
    # після правки 1 із 17).
    lines.append("- відмова: питання не про дані бази і не про норму — "
                 "погода, побутові поради, прохання щось змінити чи видалити. "
                 "Питання про строк, ліміт чи дозвіл, установлений "
                 "нормативним актом, це НЕ відмова")
    lines.append(
