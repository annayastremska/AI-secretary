...
Вони дублюють живі числа розділу «База зараз», а два числа про одне й те саме
на одному екрані -- це рівно той клас дефекту, який ми ловили весь тиждень.
(Спершу мотивація була інша: у пакеті стояло «41 нормативний акт» проти 44 у
базі. У замірі від 29.08 Андрій це підтягнув до 44, тобто застарілості вже
немає -- а згортати все одно треба, бо дублювання лишається.)
"""
import io
import json
import os
...
def test_package_is_present_and_flat():
    ...
    #: ОДИНАДЦЯТЬ від 29.08: Андрій додав метрику «конфлікти станів, що
    #: чекають ока людини». Число тут жорстке навмисно (див. шапку), і
    #: саме тому воно спрацювало: вітрина поповзла, і це стало видно
    #: падінням тесту, а не тихо.
