...
STATIC_FILES = {
    ...
    # Перемикач мови інтерфейсу. Той самий принцип, що в теми: локально, без
    # зовнішніх запитів. Переклад -- словником по DOM, тому підписи сторінок
    # не переписані в сотні місць; розбір рішення -- у самому файлі.
    "lang-toggle.js": (os.path.join(APP_DIR, "static", "lang-toggle.js"),
                       "application/javascript"),
...
#: Яку версію обличчя віддавати. Стару НЕ видаляємо (рішення Ані 27.08:
#: дати порівняти), тому перемикання -- одна змінна, а не правка файлів.
#:
#: ДЕФОЛТ v3 ІЗ 01.09. Був v1, а демо весь час ішло у v3 -- вибір робила
#: змінна APP_THEME у `.env` НА СЕРВЕРІ. Сервер вимкнули разом із `.env`,
#: і будь-хто, хто підняв би апку з репозиторію, побачив би обличчя, яке
#: ми не показували й місяць не правили, вважаючи його поточним.
APP_THEME = os.environ.get("APP_THEME", "v3").strip().lower()
...
# ІКОНКА ВКЛАДКИ ДЛЯ ЧАТА -- штатним параметром, а не лише тегом у head.
#
# Питання Ані 28.08: «чому на вкладці іконка шолома на всіх сторінках, окрім
# чату?». Дві звичайні сторінки мають `<link rel="icon">` у власній розмітці, а
# чат малює Gradio.
#
# Тег у `head` я додала, і він приїжджає -- але НЕ як розмітка: Gradio віддає
# `head` рядком у своєму конфізі й вставляє його в документ уже в браузері.
# Для стилів і скриптів це працює (перемикач теми в чаті живий), а для іконки
# покладатись на це не варто: вкладка малюється до того, як скрипт відпрацює.
#
# `favicon_path` -- штатний шлях: Gradio сам віддає файл і ставить посилання в
# серверній розмітці. Той самий файл, що на сторінках, -- інакше в однієї
# вкладки була б інша іконка.
_FAVICON = os.path.join(APP_DIR, "chat_gradio", "assets", "mark-avatar.svg")
