...
outcome: fixed — parse_rank_and_name повертає симетричний сигнал
  ...
  Прогін З МОДЕЛЛЮ (15.08, репро-docx з «кизи»): LLM-фолбек з повним рядком
  у підказці віддає patronymic='Мустафа кизи' цілком (method=llm, 0.6,
  заземлено в тексті) — ідентичність особи правильна, тиші немає.
```
...
outcome: fixed — number_from_words повертає значення лише коли РОЗПІЗНАНО
  ...
  Прогін З МОДЕЛЛЮ (15.08, репро-docx «двадцять одін»): duration_days
  resolved=False з raw_text='двадцять одін' в unresolved_values (зв'язка з
  R-B1-04) — вигаданого 20 немає, рев'юер бачить, що стояло в документі.
```
...
outcome: fixed — нова перевірка «зв'язок_скасування» у кожному записі:
  документ із пара.replaces мусить нести supersedes-зв'язок, документ без
  пари — не нести (вигаданий зв'язок теж провал). Стейл-текст звіту замінено
  фактичним станом зв'язків (+/-, номери). Тести: 3.
  Корпуси: leave docx/pdf 224/224, deployment docx/pdf 183/183, тестів 184.
```
...
outcome: fixed (перший крок за пропозицією A1) — walk_tables віддає клітинку
  блоком-словником {text, bbox: None, table, row, col}; увесь код нижче за
  течією вже вміє блоки-словники (шлях Surya/PDF), тож екстракція бачить той
  самий текст, а адреса більше не викидається. SINGLE_VALUE_DB_TARGETS не
  чіпано — багаторядкова модель фактів лишається планом
  2026-08-14_multirow-tables. Тест: test_docx_table_cells_carry_their_address.
  Корпуси: 224/224 ×2, 183/183 ×2 (жодна цифра не змінилась), тестів 188.
```
...
outcome: fixed — MAX_PLAUSIBLE_DAYS знято з коду; межі оголошує ПОЛЕ схеми
  (min_value/max_value, у day-полів лишились 1..366 у YAML — захист від
  OCR-шуму не зник, переїхав). number_from_words/ground_llm_value читають
  межі поля; валідатор еррорить межі на не-number типі й min>max.
  ДО: number('500') -> None завжди. ПІСЛЯ: 500 приймається, якщо поле не
  оголосило межу. Сирий текст відсіченого значення видно через
  unresolved_values (R-B1-04). Тести: 3. Корпуси: 224/224 ×2, 183/183 ×2,
  тестів 194.
```
...
outcome: fixed — мапа «тип -> template» переїхала в field-mapping.yaml
  (розділ doc_types, одне джерело для template_ok і check_mapping; невідомий
  тип еталона / неіснуючий шаблон — помилка мапінгу). DOC_ID_RE (LEAVE|TRIP)
  знято: ID виводиться з ключів еталона (doc_id_from_filename(name,
  known_ids)). Тести: 3. Корпуси: 224/224 ×2, 183/183 ×2, шаблон 16/16 і
  14/14, тестів 191.
```
...
outcome: not-needed — верифікатор сам констатує: «це структура, не дефект»,
  заміряного наслідку немає, циклу немає, межа subject_kind задокументована в
  самому модулі. Перекроювання імпортів зараз — churn з ризиком регресії без
  жодного вимірного виграшу; якщо цикл колись з'явиться, його зловить сам
  Python (ImportError на завантаженні), тобто тихим цей клас не є.
```
...
note: цифру «1,5-4 хв на виклик» я НЕ переміряв (це вимагало б прогону з
  ...
  моїм заміром.
outcome: skipped — це не помилка коду, а рішення продукту, якого немає.
  Механізм досяжний задокументованою опцією конфіга (self_consistency_n > 1),
  тобто «мертвим кодом» він не є; записи llm_split_vote в UNRELIABLE_METHODS
  і CONFIDENCE_BY_METHOD описують саме цю опцію. Питання до Анни (один
  рядок): видаляємо опцію self_consistency_n>1 разом із majority_vote і
  двома записами моделі довіри — чи лишаємо як капабіліті? Якщо лишаємо,
  потрібен окремий замір її користі з увімкненою моделлю.
```
...
outcome: fixed — після архівації process_target викликає
  _update_persisted_meta: збережений .md перезаписується з archived_to (той
  самий ключ, рядок індексу не дублюється, розпізнаний текст зберігається).
  Тест: test_archived_to_reaches_the_stored_record (directory-mode у tmp).
  Корпуси: 224/224 ×2, 183/183 ×2, тестів 195.
```
...
outcome: fixed (частково, свідомо) — ваги доменного скорингу названо
  (DOMAIN_TITLE_WEIGHT=3 / DOMAIN_BODY_WEIGHT=1) і задокументовано, ЧОМУ вони
  навмисно окремі від TITLE_WEIGHT/ANCHOR_WEIGHT: два скорери відповідають на
  різні питання й мусять працювати незалежно (доменний існує саме там, де
  шаблонний не спрацював); перетин фраз між довідником доменів і анкорами
  схем — навмисний з тієї самої причини. Злиття двох скорерів відхилено.
  Корпуси: 224/224 ×2, 183/183 ×2, тестів 197.
```
...
note: власне обрив процесу посеред write я не імітував — це властивість
  ...
  рядок, тому злиті.
outcome: fixed — save() пише в <key>.tmp поруч і os.replace-ить атомарно;
  tmp-хвіст прибирається у finally. Наслідок A2-13 (запис без рядка індексу
  -> другий повний запис) закритий раніше R-A1-04 (retire + supersedes).
  Тест: test_store_save_is_atomic (обірваний replace лишає попередній вміст
  цілим). Корпуси: 224/224 ×2, 183/183 ×2, тестів 196.
```
...
outcome: fixed — константа UNRESOLVED_TERM_MARKER у normalize.py,
  build_record імпортує її замість літерала. Корпуси: 224/224 ×2,
  183/183 ×2, тестів 194.
```
...
how_verified: normalize.py:429-431 — `re.sub(r"\s*за\s*\d{4}\s*рік\s*[,.;]?\s*$",
  ...
  місцях коду.
outcome: fixed (видалено) — re.sub на «за NNNN рік» знято з match_dictionary:
  правило було знанням форми в генеричному коді й фактично мертвим (єдине
  category-поле — military_rank, суфікс там неможливий). Єдиний власник
  знання — дериватор extract_year_suffix, оголошений схемою.
  Корпуси: 224/224 ×2, 183/183 ×2 (жодна цифра не змінилась), тестів 195.
```
...
outcome: fixed — прилад отримав власний читач довідника
  (load_rank_aliases читає military_rank.yaml напряму, без
  build_alias_lookup пайплайна) з тим самим обґрунтуванням, що для власної
  копії гомогліфів: спільні ДАНІ, роздільний КОД читання.
  Корпуси: 224/224 ×2, 183/183 ×2, тестів 195.
```
...
outcome: fixed — сторінка без текстового шару І без жодного зображення
  (page.get_images() порожній) тепер вважається порожнім аркушем, не сканом:
  скан за визначенням містить растр. Репро b2 (docx під іменем .pdf) дає
  source_kind=electronic без попереджень; справжні скани (растр є) — photo,
  як і були. Тест: test_borndigital_file_under_pdf_name_stays_electronic.
  Корпуси: 224/224 ×2, 183/183 ×2, тестів 197.
```
